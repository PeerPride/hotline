<script setup lang="ts">
import axios from 'axios';
import { IConversationCategory } from '@/types/conversationcategory';
</script>

<template>
  <h1>Conversation Categories</h1>
  <v-btn color="secondary" class="float-right"
    ><RouterLink
      :to="{ name: 'ConversationCategoryAdd' }"
      class="btn btn-outline-primary float-end"
    >
      Add
    </RouterLink></v-btn
  >
  <main>
    <v-table v-if="categories.length > 0" class="table table-condensed table-striped">
      <thead>
        <tr>
          <th>Name</th>
          <th>Description</th>
          <th>Status</th>
          <th></th>
        </tr>
      </thead>
      <tbody>
        <tr v-for="cat in categories" :key="cat.id">
          <td>{{ cat.attributes.name }}</td>
          <td>{{ cat.attributes.description }}</td>
          <td>{{ cat.attributes.is_active ? 'Active' : 'Inactive' }}</td>
          <td>
            <div class="btn-group" role="group" aria-label="Basic example">
              <v-btn color="info"
                ><RouterLink
                  :to="{ name: 'ConversationCategoryView', params: { id: cat.id } }"
                  class="btn btn-secondary"
                >
                  View
                </RouterLink></v-btn
              >
              <v-btn color="info"
                ><RouterLink
                  :to="{ name: 'ConversationCategoryEdit', params: { id: cat.id } }"
                  class="btn btn-outline-secondary"
                  >Edit</RouterLink
                ></v-btn
              >
            </div>
          </td>
        </tr>
      </tbody>
    </v-table>
  </main>
</template>
<script lang="ts">
export default {
  data: () => {
    return {
      categories: [] as IConversationCategory[],
      error: '' as string,
    };
  },
  computed: {
    //TODO
    listedItems() {
      return this.categories.map((item) => {
        return { ...item, description: item.attributes.description.substring(1, 30) };
      });
    },
  },
  mounted: function () {
    axios.get('/api/v1/conversationcategories').then((response) => {
      this.categories = response.data.data;
    });
  },
};
</script>
